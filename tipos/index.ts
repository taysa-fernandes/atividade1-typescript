...
//b
const usuariosAtivos: User[] = usuarios.filter(usuario => usuario.ativo);
...
//c
const usuariosMaiores25: User[] = usuarios.filter(usuario => usuario.idade > 25);
...
//d
const nomeUsuario: String [] = usuarios.map(usuario => usuario.nome);
console.log("Nomes dos usuários:");
console.log(nomeUsuario);

//e
const aumentarIdade: User [] = usuarios.map(usuario =>({
    ...usuario,
...
//g
const idadeTotal: number = usuarios.reduce((totalIdade, usuario) => totalIdade + usuario.idade, 0);
console.log("Idade total dos usuários:", idadeTotal);

//h
const totalAtivos: number = usuarios.reduce((totalAtivos, usuario) => totalAtivos + (usuario.ativo ? 1 : 0), 0);
console.log("Total Ativos:", totalAtivos);

//i
const usuarioJoao: User | undefined = usuarios.find(usuario => usuario.nome === 'João');
if (usuarioJoao) {
...
//j
const usuarioInativo: User | undefined =  usuarios.find(usuario => usuario.ativo === false);
if (usuarioInativo) {
    console.log("Usuário encontrado:");
